<script></script>

<template>
  <div class="slide" id="1">
    <div class="content first-content">
      <div class="container-fluid">
        <div class="col-12 col-sm-12 col-md-12 col-lg-3 col-xl-3 profile-image">
          <div class="author-image"><img src="/me-final.png" alt="Author Image" /></div>
        </div>
        <div class="col-12 col-sm-12 col-md-12 col-lg-9 col-xl-9 pl-5 pt-3 first-page">
          <div class="social-lg">
            <a href="https://www.linkedin.com/in/entrit-senia/"
              ><i class="fa-brands fa-linkedin"></i
            ></a>
            <a href="https://www.instagram.com/entritsenia/"
              ><i class="fa-brands fa-instagram"></i
            ></a>
            <a href="https://gr.pinterest.com/entritsenia/"
              ><i class="fa-brands fa-pinterest"></i
            ></a>
            <!--<i class="fa-brands fa-twitter"></i> -->
          </div>
          <h2>I'm Entrit Senia</h2>
          <p>
            <span>WEB DEVELOPER</span>
            <!-- <span>Vue Js</span> -->
            <!-- <span>INERTIA JS</span> -->
            <!-- <span>PHP LARAVEL</span> -->
            <span>BACK-END DEVELOPER</span>
          </p>
          <RouterLink to="/resume"
            ><button class="mr-4 first-button-index">About me</button></RouterLink
          >
          <a href="../../public/EntritSenia.pdf" download
            ><button class="second-button-index">Download CV</button></a
          >
        </div>
      </div>
      <div class="social">
        <a href="https://www.linkedin.com/in/entrit-senia/"
          ><i class="fa-brands fa-linkedin"></i
        ></a>
        <a href="https://www.instagram.com/entritsenia/"><i class="fa-brands fa-instagram"></i></a>
        <a href="https://gr.pinterest.com/entritsenia/"><i class="fa-brands fa-pinterest"></i></a>
        <!--<i class="fa-brands fa-twitter"></i> -->
      </div>
    </div>
  </div>
</template>
